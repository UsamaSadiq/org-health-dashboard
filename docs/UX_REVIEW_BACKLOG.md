...
| C4 | **Ownership is a top-level nav *section* containing one dead page.** [VERIFIED] Coverage is 0.0%; the required `ownership.owner` / `ownership.owner_name` columns do not exist in the snapshot at all (only empty `theme`/`squad`/`priority`). Shipping an empty section costs more trust than hiding it. Suggest flag-gating on data presence, not on config. | P1 |
