...
| B14 | **Rankings permuted between machines, so "Top 5" was not a fact.** [VERIFIED 2026-09-07, new] `sort_values` defaults to `kind="quicksort"`, which is not stable, and `nlargest`/`nsmallest` resolve ties by input position. With 62 repos on grade A and five movers on exactly `-25.5`, tied rows came back in an order set by the numpy build rather than the data. Caught by the visual gate: the same fixture rendered on two machines showed `openedx/XBlock` in Top 5 on one and `openedx/openedx-filters` on the other, both at 93.3, and permuted both movers tables. Every ranked surface was affected. **[RESOLVED 2026-09-07 — this branch]** `dashboard/lib/ordering.py` sorts on `(value, repo_name)` for a total order; all ranked surfaces go through it. Note this makes ties *stable*, not *visible* — B9 is still open. | **P0** |
...
| H10 | **The app fetches Inter from Google Fonts at render time, so rendering is not self-contained.** [VERIFIED 2026-09-07, new] `dashboard/ui/theme.py` `@import`s `fonts.googleapis.com/css2?family=Inter`. Every glyph the user sees — and every glyph in every committed baseline — depends on a third party being reachable. Two consequences. In production it is an availability and privacy dependency on Google for every page load (see also docs/PRIVACY.md). For the visual gate it is the one rendering input the harness cannot pin: an egress-filtered runner, a rate-limit, or an upstream change to the Inter webfont reflows every text run and reds the gate on all 14 images with no code change. Mitigated, not fixed: `_wait_for_fonts` now aborts the capture rather than silently emitting fallback-rendered PNGs. **Fix:** vendor the woff2 in-repo and serve it via `@font-face` (Inter is OFL-1.1; `scripts/uxaudit/vendor/` is the existing precedent for a vendored third-party asset with licence and checksum). Deferred because it changes production CSS and static-asset serving, which wants its own review and rebaseline. | P1 |
