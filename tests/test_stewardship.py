...
@pytest.mark.parametrize(
    "row, expected",
    [
        (_unmaintained("a"), stewardship.UNMAINTAINED),
...
def test_thin_ownership_without_warnings_is_not_listed():
    scored = pd.DataFrame([_unmaintained("openedx/healthy")])

    assert stewardship.at_risk_repos(scored, None, now=NOW).empty


def test_each_warning_is_reported():
    scored = pd.DataFrame([_unmaintained("openedx/ora", activity=10.0, push=OLD_PUSH, score=40.0)])
...
def test_repo_missing_from_baseline_has_no_delta():
    scored = pd.DataFrame([_unmaintained("openedx/new", activity=10.0)])
...
def test_catalog_link_uses_repo_basename():
    scored = pd.DataFrame([_unmaintained("openedx/edx-ora2", activity=10.0)])
